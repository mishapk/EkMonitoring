  ...
  while(!ADO->Eof)
  {
    ...
    //--------------------------------
      ...
      if(ADO->Fields->FieldByName("Date_type")->AsInteger==1)
     {
       ...
       CompareAlarm(ADO->Fields->FieldByName("TAG.ID")->AsInteger,value); //Проверка Алармов
...
void __fastcall TComPort::CompareAlarm(int TagID, float value)
{
  ...
  ADOH = new TADOTable(NULL);
  ADO->Connection=DataModule2->ADOConnection;
  ADOH->Connection=DataModule2->ADOConnection;
  ADOH->TableName="ALARM_HISTORY";
  ADOH->Active=true;
  ADO->SQL->Clear();
  ADO->SQL->Add("SELECT *");
  ADO->SQL->Add("FROM ALARMS");
  ADO->SQL->Add("WHERE ID_TAG="+IntToStr(TagID));
  ADO->Active=true;
  ADO->First();
  while(!ADO->Eof)
  {
        ...
        if(rez && !trigger)
        {
                ...
                ADOH->Insert();
                ADOH->Fields->FieldByName("ID_ALARMS")->AsInteger=ADO->Fields->FieldByName("ID")->AsInteger;
                ADOH->Fields->FieldByName("DATETIME")->AsString=DateTimeToStr(Now());
                ADOH->Fields->FieldByName("STATUS")->AsBoolean=true;
                ADOH->Fields->FieldByName("Values")->AsFloat=value;
                ADOH->Post();
                Synchronize(UpdateHistoryTable);
        }
        if(!rez && trigger)
        {
                ADO->Edit();
                ...
                ADOH->Fields->FieldByName("ID_ALARMS")->AsInteger=ADO->Fields->FieldByName("ID")->AsInteger;
                ADOH->Fields->FieldByName("DATETIME")->AsString=DateTimeToStr(Now());
                ADOH->Fields->FieldByName("STATUS")->AsBoolean=false;
                ADOH->Fields->FieldByName("Values")->AsFloat=value;
                ADOH->Post();
              Synchronize(UpdateHistoryTable);
        }
...
void __fastcall TComPort::UpdateHistoryTable()
{
   DataModule2->ADOTableAlarmHistory->Active=false;
   DataModule2->ADOTableAlarmHistory->Active=true;
